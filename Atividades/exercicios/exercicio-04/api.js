...
function preencherSelectSubdistritos(data){
...
function carregarSubdistritos(){

    const cidades = document.getElementById("cidades");
    const cidade_index = cidades.selectedIndex;
    const cidade_id = cidades.options [cidade_index].value;

    const subdistritos = document.getElementById("cidades");
    limparSelect(subdistritos)

    if (cidade_id == ""){
        return;
    }

    const url_subdistrito = `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${cidade_id}/subdistritos`;

    fetch(url_subdistrito)
        .then(response => response.json())
        .then(data => preencherSelectSubdistritos(data))
        .catch(error => console.error(error))
